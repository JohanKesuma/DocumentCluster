...
//        ArrayList<Term> terms = getDictionary();
     ...
     */
    public void setListOfCluster(ArrayList<Cluster> listOfCluster) {
    ...
    public void preClustering() {
        ...
        for (int i = 0; i < documents.size(); i++) {
            // baca idDoc
            int idDoc = documents.get(i).getId();
            // buat posting dengan nilai TF-IDFnya
            documents.get(i).setListOfClusteringPosting(makeTFIDF(idDoc));
    ...
    public void clustering() {
        ...
        for (int i = 0; i < NUMBER_OF_DOCUMENT_CLUSTER; i++) {
            ...
            cluster.setCenter(documents.get(i+1));
        ...
        while (move) {
            ...
            for (int i = 0; i < documents.size(); i++) {
                ...
                for (int j = 0; j < listOfCluster.size(); j++) {
                    double sim = getCosineSimilarity(documents.get(i).getListOfClusteringPosting(),
                ...
                Cluster currentCluster = listOfSimilarity.get(0).getCluster();
                System.out.println(doc.getId() + " -> "+currentCluster.getIdCluster() + " : "
                        +listOfSimilarity.get(0).getSimilarity() + " || " + listOfSimilarity.get(1).getSimilarity());
                // jika dokumen pindah cluster
                if (doc.getIdCluster() != currentCluster.getIdCluster()) {
                    move = true;
                    if (doc.getIdCluster() != -1) {
                        ArrayList<Document> clusterMembers = listOfCluster.get(doc.getIdCluster()).getMember();
                        Collections.sort(clusterMembers);
                        int position = Collections.binarySearch(clusterMembers, doc);
                        if (position >= 0) {
                            clusterMembers.remove(position);
                        }
                    }
                    System.out.print("doc : " +doc.getId() + ",  "+doc.getIdCluster() + " -> ");
                    doc.setIdCluster(currentCluster.getIdCluster());
                    System.out.println(doc.getIdCluster());
                    currentCluster.getMember().add(doc);
                }
            }
            ...
            for (int i = 0; i < NUMBER_OF_DOCUMENT_CLUSTER; i++) {
                double[] terms = new double[dictionary.size()];
                ArrayList<Document> memberDocument = listOfCluster.get(i).getMember();
                for (int j = 0; j < memberDocument.size(); j++) {
                    Document doc = memberDocument.get(j);
                    for (int k = 0; k < terms.length; k++) {
                        int search = Collections.binarySearch(doc.getListOfClusteringPosting(),
                                dictionary.get(k).getTermList().getPostings().get(0));

                        if (search >= 0) {
                            terms[k] += doc.getListOfClusteringPosting().get(search).getWeight();
                        }

                    }
                }
                ArrayList<Posting> postingCluster = new ArrayList<>();
                for (int j = 0; j < terms.length; j++) {
                    Posting posting = new Posting();
                    posting.setTerm(dictionary.get(j).getTerm());
                    double average = terms[j] / listOfCluster.get(i).getMember().size();
                    posting.setWeight(average);
                    postingCluster.add(posting);
                }
                Document centroid = new Document();
                centroid.setId(-1);
                centroid.setListOfClusteringPosting(postingCluster);
                listOfCluster.get(i).setCenter(centroid);
            }
        }
    }
